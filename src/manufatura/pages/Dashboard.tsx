...
export function Dashboard({ goto }: { goto: (s: 'estoque' | 'producao') => void }) {
  ...
  const disp = useMemo(() => RECEITAS.map(r => ({ receita: r, d: disponibilidade(r, itens) })), [itens])
  ...
  const valorPorTipo = useMemo(() => {
    const g = { materia_prima: 0, embalagem: 0, produto_acabado: 0 }
    itens.forEach(i => { g[i.tipo] += valorEstoque(i) })
    return [
      { nome: 'Matéria-prima', valor: g.materia_prima, cor: 'hsl(40 73% 55%)' },
      { nome: 'Embalagem', valor: g.embalagem, cor: 'hsl(40 66% 54%)' },
      { nome: 'Produto acabado', valor: g.produto_acabado, cor: 'hsl(140 26% 56%)' },
    ]
  }, [itens])
          ...
          {disp.map(({ receita, d }) => {
            ...
            return (
              <Card key={receita.id} className="p-5 relative overflow-hidden">
                ...
                <div className="mt-4 pt-3.5 border-t border-[hsl(var(--gold)/0.08)] text-xs text-text-secondary relative">
...
function Kpi({ label, value, unit, sub, children, icon, tone, accent, onClick }: {
  ...
  return (
    <button onClick={onClick} disabled={!onClick}
      className={`gradient-card rounded-2xl border border-[hsl(var(--gold)/0.10)] p-4 text-left ${onClick ? 'hover:border-[hsl(var(--gold)/0.28)] transition cursor-pointer' : 'cursor-default'}`}>
...
function ReporRow({ item, comprar }: { item: Item; comprar: number }) {
  return (
    <div className="flex items-center gap-3 py-2.5 border-b border-[hsl(var(--gold)/0.06)] last:border-0">
