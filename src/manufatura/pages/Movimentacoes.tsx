    ...
    <div className="animate-fade-up">
      <div className="flex items-end gap-3 mb-5 flex-wrap">
        ...
        <button onClick={() => setNovo(true)}
          className="ml-auto flex items-center gap-1.5 h-9 px-3.5 rounded-lg gradient-gold text-[hsl(30_14%_8%)] font-semibold text-sm hover:brightness-110 transition">
      ...
      <div className="flex items-center gap-2 mb-4 flex-wrap">
        {FILTROS.map(f => (
          <button key={f.id} onClick={() => setFiltro(f.id)}
            className={`h-8 px-3.5 rounded-full text-xs font-medium transition border ${filtro === f.id ? 'text-gold border-[hsl(var(--gold)/0.4)]' : 'text-text-secondary border-[hsl(var(--gold)/0.1)] hover:text-foreground'}`}
        ...
        <div className="relative ml-auto w-full sm:w-56 mt-2 sm:mt-0">
          ...
          <input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Buscar item…"
            className="w-full h-8 pl-9 pr-3 rounded-lg bg-[hsl(var(--surface-overlay))] border border-[hsl(var(--gold)/0.1)] text-sm outline-none focus:border-[hsl(var(--gold)/0.4)] transition" />
        </div>
      </div>
        ...
        {grupos.map(([dia, movs]) => (
          <div key={dia}>
            ...
            {movs.map(m => {
              ...
              return (
                <div key={m.id} className="flex items-center gap-3 px-5 py-3 border-t border-[hsl(var(--gold)/0.05)]">
...
function NovoMovimento({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className="relative w-full max-w-md surface-float rounded-2xl gold-glow animate-slide-up p-5" onClick={e => e.stopPropagation()}>
        ...
        <select value={itemId} onChange={e => setItemId(e.target.value)}
          className="w-full h-10 px-3 rounded-lg bg-[hsl(var(--surface-overlay))] border border-[hsl(var(--gold)/0.14)] text-sm outline-none focus:border-[hsl(var(--gold)/0.4)] mb-4">
        ...
        <div className="grid grid-cols-4 gap-1.5 mb-4">
          {TIPOS.map(t => (
            <button key={t.id} onClick={() => setTipo(t.id)}
              className={`h-9 rounded-lg text-xs font-medium transition border ${tipo === t.id ? 'text-gold border-[hsl(var(--gold)/0.45)]' : 'text-text-secondary border-[hsl(var(--gold)/0.1)] hover:text-foreground'}`}
        ...
        <input type="number" min={0} step={item?.uom === 'un' ? 1 : 0.1} value={qtd} onChange={e => setQtd(Math.max(0, parseFloat(e.target.value) || 0))}
          className="w-full h-11 px-3 rounded-lg bg-black/30 border border-[hsl(var(--gold)/0.16)] font-display text-2xl tnum outline-none focus:border-[hsl(var(--gold)/0.5)] mb-4 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" />
        ...
        <input value={motivo} onChange={e => setMotivo(e.target.value)} placeholder="ex.: quebra, contagem, venda balcão…"
          className="w-full h-10 px-3 rounded-lg bg-[hsl(var(--surface-overlay))] border border-[hsl(var(--gold)/0.14)] text-sm outline-none focus:border-[hsl(var(--gold)/0.4)] mb-5" />

        <button onClick={registrar} disabled={qtd <= 0}
          className={`w-full h-11 rounded-xl font-semibold transition ${qtd > 0 ? 'gradient-gold text-[hsl(30_14%_8%)] hover:brightness-110' : 'bg-[hsl(var(--surface-overlay))] text-text-muted cursor-not-allowed'}`}>
          Registrar
        </button>
      </div>
    </div>
