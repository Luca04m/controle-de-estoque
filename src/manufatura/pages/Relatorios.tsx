...
const CAT_LABEL: Record<CategoriaItem, string> = {
  liquido: 'Líquidos', po: 'Pós', aditivo: 'Aditivos', garrafa: 'Garrafas', rotulo: 'Rótulos',
  ...
  return (
    <div className="animate-fade-up space-y-7">
      ...
      <div className="grid lg:grid-cols-12 gap-4">
        {/* Curva ABC */}
        <section className="lg:col-span-5 rounded-2xl border border-[hsl(var(--gold)/0.1)] gradient-card p-5">
          ...
          <div className="grid grid-cols-3 gap-2 mb-5">
            {abc.dist.map(d => (
              <div key={d.classe} className="rounded-xl p-3 text-center" style={{ background: `hsl(${ABC_COR[d.classe]}/0.08)`, border: `1px solid hsl(${ABC_COR[d.classe]}/0.2)` }}>
        ...
        {/* Valor por categoria */}
        <section className="lg:col-span-7 rounded-2xl border border-[hsl(var(--gold)/0.1)] gradient-card p-5">
      ...
      {/* Itens em risco */}
      <section className="rounded-2xl border border-[hsl(var(--gold)/0.1)] gradient-card overflow-hidden">
        <div className="px-5 py-4 border-b border-[hsl(var(--gold)/0.08)] flex items-center gap-2">
        ...
        <div className="divide-y divide-[hsl(var(--gold)/0.06)]">
          {risco.map(({ item, dias }) => (
...
function MiniKpi({ icon, label, value, sub, tone }: { icon: React.ReactNode; label: string; value: string; sub?: string; tone?: 'warn' }) {
  ...
  return (
    <div className="gradient-card rounded-2xl border border-[hsl(var(--gold)/0.1)] p-4">
